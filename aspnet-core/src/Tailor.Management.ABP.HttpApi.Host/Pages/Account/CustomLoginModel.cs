...
public class CustomLoginModel : AccountPageModel
{
    ...
    public virtual async Task<IActionResult> OnPostAsync(string action)
    {
        ...
        if (user == null)
        {
            Alerts.Danger("Invalid email address or password");
            return Page();
        }
    ...
    {
        if (!ValidationHelper.IsValidEmailAddress(LoginInput.Email))
        {
            return;
        }

        var userByUsername = await UserManager.FindByNameAsync(LoginInput.Email);
        if (userByUsername != null)
        {
            return;
        }

        var userByEmail = await UserManager.FindByEmailAsync(LoginInput.Email);
        if (userByEmail == null)
        {
            return;
        }

        LoginInput.Email = userByEmail.UserName;
    }
